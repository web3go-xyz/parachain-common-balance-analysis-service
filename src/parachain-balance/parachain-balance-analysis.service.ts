...
import { Transfers } from 'src/common/entity/ParachainBalanceModule/Transfers';
import { TransactionTypeEnum } from 'src/viewModel/parachain-balance/TransactionTypeEnum';
import { ReservRepatriateds } from 'src/common/entity/ParachainBalanceModule/ReservRepatriateds';
import { Slashes } from 'src/common/entity/ParachainBalanceModule/Slashes';
import { Withdraws } from 'src/common/entity/ParachainBalanceModule/Withdraws';
import { Unreserveds } from 'src/common/entity/ParachainBalanceModule/Unreserveds';
import { Reserveds } from 'src/common/entity/ParachainBalanceModule/Reserveds';
import { Deposits } from 'src/common/entity/ParachainBalanceModule/Deposits';
import { BalanceSets } from 'src/common/entity/ParachainBalanceModule/BalanceSets';
import { Endoweds } from 'src/common/entity/ParachainBalanceModule/Endoweds';
import { AccountsLatestSyncBlock } from 'src/common/entity/ParachainBalanceModule/AccountsLatestSyncBlock';
import { Cron } from '@nestjs/schedule';
import { ApiPromise, WsProvider } from '@polkadot/api';
import { AccountInfo } from "@polkadot/types/interfaces/system";
import { CronConstants } from 'src/cron.constants';
import { AppConfig } from 'src/common/setting/appConfig';

@Injectable()
export class ParachainBalanceAnalysisService {
  ...
  @Cron(CronConstants.ACCOUNT_BALANCE_INTERVAL)
  async syncAccountsBalance(): Promise<any> {
    if (this.isRunning) {
    ...
    const latestSyncBlock: AccountsLatestSyncBlock = await this.balanceAccountsLatestSyncBlockRepository.findOne();
    let latestSyncBlockNum;
    let ws_endpoint = AppConfig.ParachainEndpoint;
    let chain_name = AppConfig.ParachainName;
    ...
    if (!latestSyncBlock) {
      ...
      newAccountsLatestSyncBlock.chain_name = chain_name;
      newAccountsLatestSyncBlock.ws_endpoint = ws_endpoint;
      newAccountsLatestSyncBlock.sync_time = new Date();
      await this.balanceAccountsLatestSyncBlockRepository.save(newAccountsLatestSyncBlock);
    } else {
    ...
    let latestBlockNumIndexed = 0;
    if (rawData1 && rawData1.length > 0) {
      const record = rawData1[0];
      latestBlockNumIndexed = record.max_block_number as number;
      ...
      while (currentIndex < total) {
        ...
        try {
          ...
          for (let rawIndex = 0; rawIndex < rawList.length; rawIndex++) {
            ...
            acc.atBlock = latestBlockNumIndexed.toString();
    ...
    this.logger.verbose("update latest sync block to " + latestBlockNumIndexed);
    await this.balanceAccountsLatestSyncBlockRepository.update(
      { id: '1' },
  ...
  constructor(
    @Inject(RepositoryConsts.PARACHAIN_BALANCE_ACCOUNT_REPOSITORY)
    private accountRepository: Repository<Accounts>,

    @Inject(RepositoryConsts.PARACHAIN_BALANCE_ENDOWEDS_REPOSITORY)
    private endowedRepository: Repository<Endoweds>,

    @Inject(RepositoryConsts.PARACHAIN_BALANCE_TRANSFERS_REPOSITORY)
    private transfersRepository: Repository<Transfers>,

    @Inject(RepositoryConsts.PARACHAIN_BALANCE_BALANCE_SETS_REPOSITORY)
    private balanceSetRepository: Repository<BalanceSets>,

    @Inject(RepositoryConsts.PARACHAIN_BALANCE_DEPOSITS_REPOSITORY)
    private depositRepository: Repository<Deposits>,

    @Inject(RepositoryConsts.PARACHAIN_BALANCE_RESERVREPATRIATEDS_REPOSITORY)
    private reservRepatriatedRepository: Repository<ReservRepatriateds>,

    @Inject(RepositoryConsts.PARACHAIN_BALANCE_RESERVEDS_REPOSITORY)
    private reservedRepository: Repository<Reserveds>,

    @Inject(RepositoryConsts.PARACHAIN_BALANCE_SLASH_REPOSITORY)
    private slashRepository: Repository<Slashes>,

    @Inject(RepositoryConsts.PARACHAIN_BALANCE_UNRESERVEDS_REPOSITORY)
    private unreservedRepository: Repository<Unreserveds>,

    @Inject(RepositoryConsts.PARACHAIN_BALANCE_WITHDRAWS_REPOSITORY)
    private withdrawRepository: Repository<Withdraws>,

    @Inject(RepositoryConsts.PARACHAIN_BALANCE_ACCOUNTS_LATEST_SYNC_BLOCK_REPOSITORY)
    private balanceAccountsLatestSyncBlockRepository: Repository<AccountsLatestSyncBlock>,
  ) {
    this.logger = new W3Logger('ParachainBalanceAnalysisService');
  }
}
